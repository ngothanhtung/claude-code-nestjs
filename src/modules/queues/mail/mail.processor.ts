...
import { OnQueueActive, Process, Processor, OnQueueCompleted, OnQueueProgress, OnQueueWaiting } from '@nestjs/bull';
...
export class MailProcessor {
  ...
  onProgress(job: Job, progress: number) {
    this.logger.debug(`Progress: ${progress}% for job ${job.id} with data ${JSON.stringify(job.data)}`);
  }
