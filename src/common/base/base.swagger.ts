...
export function ApiGet($ref: any, summary?: string) {
  return applyDecorators(ApiOperation({ summary }), OkResponse($ref, true));
}

export function ApiGetAll($ref: any, name?: string) {
  return applyDecorators(ApiOperation({ summary: 'Get all ' + name }), OkResponse($ref, false));
}
